...
| D18 | 2026-08-09 | `Morkeeth/nullspace-dbt` is the public fulfillment remote. A real `https://github.com/...` PR requires push credentials (`NULLSPACE_DBT_TOKEN` or collaborator write). Without them `pr_url` stays honest `file://`. When a real PR opens, the ghost stays `claimed` until merge; `nullspace.cli finalize` solidifies. | **DECIDED — write still blocked for cursor[bot]** | Cursor Lane A | `gh repo view` PUBLIC; `git push` → 403; `builder.create_change_reference` |
